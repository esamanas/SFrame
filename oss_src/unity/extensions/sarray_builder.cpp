...
void sarray_builder::init(size_t num_segments, size_t history_size, flex_type_enum dtype) {
  if(m_inited)
...
void sarray_builder::append(const flexible_type &val, size_t segment) {
  if(!m_inited)
...
void sarray_builder::append_multiple(const std::vector<flexible_type> &vals, size_t segment) {
  for(const auto &i : vals) {
    this->append(i, segment);
  }
}

flex_type_enum sarray_builder::get_type() {
  return m_ary_type;
}

std::vector<flexible_type> sarray_builder::read_history(size_t num_elems) {
  if(!m_inited)
    log_and_throw("Must call 'init' first!");
